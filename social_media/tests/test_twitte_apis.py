...
class TestManageTwittesAPI:
    ...
    # def test_manage_twittes_list(self, profile_twitte, setup_manage):
    #     baker.make(Twitte, profile=profile_twitte, _quantity=5)
    #     response = self.client.get(reverse("social_media:manage-twittes-list"))
    #     assert response.status_code == status.HTTP_200_OK
    #     assert len(response.data['results']) == 5

    # def test_delete_twitte(self, profile_twitte, setup_manage):
    #     twitte = baker.make(Twitte, profile=profile_twitte)
    #     response = self.client.delete(reverse("social_media:manage-twittes-detail", kwargs={"pk": twitte.id}))
    #     assert response.status_code == status.HTTP_204_NO_CONTENT
    #     assert not Twitte.objects.filter(pk=twitte.id).filter(display=True).exists()
...
class TestTwitteChartAPI:
    ...
    # def test_last_week_tweets(self, profile_twitte, setup):
    #     # Create tweets for last week
    #     for i in range(7):
    #         baker.make(Twitte, profile=profile_twitte, created_at=timezone.now() - datetime.timedelta(days=i))
    #     response = self.client.get(reverse("social_media:tweets-charts"))
    #     assert response.status_code == status.HTTP_200_OK
