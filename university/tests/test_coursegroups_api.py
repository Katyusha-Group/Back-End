...
class TestCoursegroups:
    ...
    def sorted_course_with_first_course_base_course_id(self, courses):
        courses_with_first_base_course_id = Course.objects.filter(
            base_course_id=courses[0].base_course_id,
            semester=project_variables.CURRENT_SEMESTER,
            sex__in=["B", "M"],
        ).all()

        sorted_courses = sorted(
            courses_with_first_base_course_id,
            key=lambda x: (
                x.capacity - x.registered_count <= 0,
                x.capacity - x.registered_count,
    ...
    def test_if_get_request_for_logged_in_user_is_status_200(
        self, api_client, user_instance, courses, course_groups_view_url
    ):
    ...
    def test_if_get_request_for_base_course_with_no_course_is_status_400(
        self, api_client, user_instance, course_groups_view_url
    ):
    ...
    def test_if_get_request_for_nonexistent_base_course_is_status_400(
        self, api_client, user_instance, course_groups_view_url
    ):
        api_client.force_login(user_instance)
        response = api_client.get(course_groups_view_url(1))
    ...
    def test_if_get_request_for_anonymous_user_is_status_403(
        self, api_client, user_instance, course_groups_view_url
    ):
        baker.make(BaseCourse, course_number=1)
        response = api_client.get(course_groups_view_url(1))
    ...
    def test_if_returned_data_has_correct_number_of_fields(
        self, api_client, user_instance, course_groups_view_url, courses
    ):
        api_client.force_login(user_instance)
    ...
    def test_if_returned_data_has_correct_fields(
        self, api_client, user_instance, course_groups_view_url, courses
    ):
        api_client.force_login(user_instance)
    ...
    def test_if_returned_data_has_correct_values_for_simple_fields(
        self,
        api_client,
        user_instance,
        ...
        for course_data, course in zip(
            response.data, sorted_course_with_first_course_base_course_id
        ):
            assert course_data["base_course_id"] == course.base_course_id
            assert course_data["capacity"] == course.capacity
            assert (
                course_data["color_intensity_percentage"]
                == course.color_intensity_percentage
            )
            assert (
                course_data["complete_course_number"] == course.complete_course_number
            )
    ...
    def test_if_returned_data_has_correct_values_for_course_times(
        self,
        api_client,
        user_instance,
        course_groups_view_url,
        ...
        for course_data, course in zip(
            ...
            for course_time_data, course_time in zip(
                course_data["course_times"], course.course_times.all()
            ):
                start = int((str(course_time.start_time))[:2])
                if start in project_variables.start_time_mapper:
                    course_time_representation = project_variables.start_time_mapper[
                        start
                    ]
                else:
                ...
                assert course_time_data["course_start_time"] == str(
                ...
                assert str(course_time_data["course_time_representation"]) == str(
                    course_time_representation
                )

    def test_if_returned_data_has_correct_values_for_exam_times(
        self,
        api_client,
        user_instance,
        course_groups_view_url,
        ...
        for course_data, course in zip(
            ...
            for exam_time_data, exam_time in zip(
                course_data["exam_times"], course.exam_times.all()
            ):
                assert (
                    str(exam_time_data["date"]).split(".")[0]
                    == str(exam_time.date).split(".")[0]
                )
                assert (
                    str(exam_time_data["exam_start_time"]).split(".")[0]
                    == str(exam_time.start_time).split(".")[0]
                )
                assert (
                    str(exam_time_data["exam_end_time"]).split(".")[0]
                    == str(exam_time.end_time).split(".")[0]
                )

    def test_if_returned_data_has_correct_values_for_teachers(
        self,
        api_client,
        user_instance,
        course_groups_view_url,
        ...
        for course_data, course in zip(
            ...
            for teacher_data, teacher in zip(
                course_data["teachers"], course.teachers.all()
            ):
    ...
    def test_if_returned_data_only_contains_courses_with_base_course_id(
        self,
        api_client,
        user_instance,
        course_groups_view_url,
        ...
        response = api_client.get(
            course_groups_view_url(
                sorted_course_with_first_course_base_course_id[0].base_course_id
            )
        )

        assert all(
            course["base_course_id"]
            == sorted_course_with_first_course_base_course_id[0].base_course_id
            for course in response.data
        )

    def test_if_returned_data_only_contains_courses_with_correct_gender(
        self,
        api_client,
        user_instance,
        course_groups_view_url,
        ...
        baker.make(
            Course,
            base_course=sorted_course_with_first_course_base_course_id[0].base_course,
            sex="F",
        )
        baker.make(
            Course,
    ...
    def test_if_returned_data_has_correct_color_intensity(
        ...
        assert all(
            response_course["color_intensity_percentage"]
            == course.color_intensity_percentage
            for response_course, course in zip(
                response.data, sorted_course_with_first_course_base_course_id
            )
        )

    def test_if_returned_data_has_correct_is_allowed(
        self,
        api_client,
        user_instance,
        course_groups_view_url,
        ...
        assert all(
            response_course["is_allowed"]
            == course.allowed_departments.filter(
